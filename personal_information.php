...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['reg'])) {
        ...
        $sno = $_SESSION['user'];
        $query = "SELECT sname,ssex,sdept,semail FROM tb_student WHERE sno=$sno";
        ...
        $query = "UPDATE tb_student SET sname='$sname',ssex='$gender',sdept='$sdept',semail='$semail' WHERE sno='$sno'";
        $result = $mysqli->query($query);
        if (!$result) {
            echo "SQL语句执行失败！";
            return;
